...
], function (BaseObject, Log, GroupHeaderListItem) {
	"use strict";

	return BaseObject.extend("recipe.RecipeEditor.controller.ListSelector", {
		 ...
		 * @class
		 * @public
		 * @alias recipe.RecipeEditor.controller.ListSelector
		 */
		 ...
		 * @return {CRUD.sampleCRUD.model.ListSelector} the list selector object for method chaining
		 * @public
		 */
					...
					if (!(oItem instanceof GroupHeaderListItem)) {
						...
						if (bFound) {
							if (i === iItemsCount - 1) { //the current item is the last but not the only one
								if (!(aItems[i - 1] instanceof GroupHeaderListItem)) {
									return aItems[i - 1];
								} else {
									return null;
								}
